 ...
 */
public class FlamerEffect extends BaseCombatLayeredRenderingPlugin implements OnFireEffectPlugin,
																					OnHitEffectPlugin,
	...
	protected List<FlamerEffect> trails;
	public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
		if (trails == null) return;

		Iterator<FlamerEffect> iter = trails.iterator();
		while (iter.hasNext()) {
...
//								   rampUp, dur, inverted);
	...
	public void onFire(DamagingProjectileAPI projectile, WeaponAPI weapon, CombatEngineAPI engine) {
		String prevKey = "shpe_flamer_prev_" + weapon.getShip().getId() + "_" + weapon.getSlot().getId();
		DamagingProjectileAPI prev = (DamagingProjectileAPI) engine.getCustomData().get(prevKey);

		FlamerEffect trail = new FlamerEffect(projectile, prev);
		...
		if (trails == null) {
			trails = new ArrayList<FlamerEffect>();
		}
		trails.add(0, trail);
	}
...
//			offset.y += vel.y * 1f;
	...
	public FlamerEffect(DamagingProjectileAPI proj, DamagingProjectileAPI prev) {
		this.proj = proj;
		this.prev = prev;
