    ...
    public static void spawnLanceBeam(CombatEngineAPI engine, Vector2f start, float angle, float range, Vector2f drift, SpriteAPI coreSprite, Color coreColorStart, Color coreColorEnd, Color coreColorFade, float coreWidthStart, float coreWidthEnd, float coreGrowthFactor, float coreTexLength, float coreTexScroll, float coreOpacityStart, float coreOpacityEnd, float coreIntro, float coreFull , float coreFade, SpriteAPI fringeSprite, Color fringeColorStart, Color fringeColorEnd, Color fringeColorFade, float fringeWidthStart, float fringeWidthEnd, float fringeGrowthFactor, float fringeTexLength, float fringeTexScroll, float fringeOpacityStart, float fringeOpacityEnd, float fringeIntro, float fringeFull , float fringeFade, float blendIn, float blendOut,  float damage, DamageType damageType, float emp, float impactSize, float force, ShipAPI source, boolean ignoreShields, boolean softFlux, boolean pierceSoftTargets, OnHitEffectPlugin onHit) {
        ...
        if (!entities.isEmpty()) {
            for (CombatEntityAPI entity : entities) {
                ...
                } if ((entity instanceof CombatAsteroidAPI || (entity instanceof MissileAPI && entity.getOwner() != source.getOwner()) || isHitableFighter) && CollisionUtils.getCollides(start, beamEnd, entity.getLocation(), entity.getCollisionRadius())) {
                    Vector2f piercePoint = MagicFakeBeam.getCollisionPointOnCircumference(start, beamEnd, entity.getLocation(), entity.getCollisionRadius());
                    log.info(entity.getMass());
                    if (pierceSoftTargets && (Math.random() * 400) > entity.getMass()) {
                        pierced.add(new PierceHitData(entity, piercePoint));
                    } else {
        ...
        //apply damage and onhits to pierced targets if necessary
        for (PierceHitData pierce : pierced) {
            CombatEntityAPI pierceTarget = pierce.target;
            Vector2f point = pierce.point;
            float hitmult = (float) Math.random();

            if (pierceTarget != null && !(damage == 0 && emp == 0)){
                engine.applyDamage(
                        pierceTarget,
                        point,
                        damage * hitmult,
                ...
                engine.addHitParticle(
                        point,
                        new Vector2f(),
                        ((float) Math.random() * impactSize / 2 + impactSize) * hitmult,
                        hitmult,
                        (fringeFull + fringeFade) * (hitmult * 0.5f + 0.5f),
                        fringeColorFade == null ? fringeColorEnd : fringeColorFade
                );
                engine.addHitParticle(
                        point,
                        new Vector2f(),
                        ((float) Math.random() * impactSize / 4 + impactSize / 2) * hitmult,
                        hitmult,
                        coreFull * (hitmult * 0.5f + 0.5f),
                        coreColorEnd
                );
            }
        }
